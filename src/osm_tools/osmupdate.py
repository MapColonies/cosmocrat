...
def get_changes_from_timestamp(input_timestamp, change_format=definitions.FORMATS_MAP['OSC']):
    temp_output_name = f'{uuid4()}.{change_format}'
    output_path = os.path.join(definitions.OSMCHANGES_PATH, change_format, temp_output_name)
    ...
    new_output_path = os.path.join(definitions.OSMCHANGES_PATH, change_format, output_name)
    os.rename(output_path, new_output_path)
    return new_output_path

def get_changes_from_file(input_path, change_format=definitions.FORMATS_MAP['OSC']):
    temp_output_name = f'{uuid4()}.{change_format}'
